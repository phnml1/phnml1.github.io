...
export const metadata: Metadata = {
  ...
  description: 'React와 TypeScript로 업무 화면과 서버 상태를 구현하고, TypeScript·SQL 기반 서버 구현 경험을 함께 정리한 이주영의 프론트엔드 포트폴리오',
...
const featuredProjects = [
  {
    ...
    summary: 'GitHub PR 댓글 이벤트를 TanStack Query 캐시에 직접 반영해 실시간 협업 흐름과 API 요청 비용을 개선한 개인 프로젝트입니다.',
  },
  {
    ...
    summary: 'PDA, Android WebView, WMS 계약을 연결해 스캐너 입력, 오프라인 복구, 부분 피킹 흐름을 다룬 실무 프로젝트입니다.',
  },
  {
    title: 'Fly:On',
    ...
    summary: 'React Native 일정 Drag & Drop을 직접 구현하고 React Profiler 측정으로 렌더링 병목을 줄인 팀 프로젝트입니다.',
  },
];
...
const additionalProjects = [
  {
    ...
    summary: '지원 단계, 일정, 알림을 하나의 PWA 흐름으로 묶고 상태 동기화 규칙과 캐시 갱신 경로를 분리한 프로젝트입니다.',
  },
  {
    title: 'NP-OIS',
    ...
    metric: '59',
    label: 'backend tests',
    summary: 'Excel VBA 주문 처리 업무를 업로드, 변환 큐, 검수, 산출물 보관이 가능한 웹 워크플로로 전환한 실무 프로젝트입니다.',
  },
  {
    title: "Runner's High",
    ...
    metric: '10s',
    label: 'pace window',
    summary: '실시간 위치 공유, 러닝 상태 분리, GPS pace 보정, 인증 복구 흐름을 다룬 위치 기반 모바일 프로젝트입니다.',
  },
];
        ...
        <div className="mx-auto w-full max-w-[1440px]">
          ...
          <div className="grid gap-8 lg:grid-cols-2">
            {featuredProjects.map((project, index) => (
              <Link
                key={project.href}
                href={project.href}
                className={`group min-w-0 rounded-xl border border-transparent bg-surface-container p-8 transition-colors duration-200 hover:border-primary/40 hover:bg-surface-high ${
                  index === 1 ? 'lg:mt-24' : ''
                }`}
              >
                <article className="grid min-w-0 min-h-[360px] grid-rows-[1fr_auto]">
                  <div>
                  ...
                  <div className="mt-12 flex items-end justify-between gap-6">
                    <div>
                      <div className="text-gradient break-all text-[clamp(2.25rem,11vw,3.75rem)] font-black tracking-[-0.06em]">{project.metric}</div>
                      <div className="font-label text-xs uppercase tracking-[0.2em] text-text-secondary">{project.label}</div>
                    </div>
                    <span aria-hidden="true" className="text-3xl text-primary transition-transform group-hover:translate-x-2">→</span>
                  </div>
                </article>
              </Link>
            ))}
          ...
          <details className="group mt-10 rounded-2xl border border-outline/70 bg-surface-container p-5">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-4">
              <div>
                <span className="font-label text-xs font-bold uppercase tracking-[0.22em] text-primary">More Projects</span>
                <p className="mt-2 text-sm leading-7 text-text-secondary">
                  주문 처리 웹 전환, PWA, 위치 기반 프로젝트는 펼쳐서 확인할 수 있습니다.
                </p>
              </div>
              <span aria-hidden="true" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-primary/50 text-lg text-primary transition-transform group-open:rotate-180">
                ▾
              </span>
            </summary>
                ...
                >
                  <article className="grid min-h-[250px] grid-rows-[1fr_auto]">
                    ...
                    <div className="mt-8 flex items-end justify-between gap-4 border-t border-outline/70 pt-5">
                      <div>
                        <div className="text-gradient text-3xl font-black tracking-[-0.04em]">{project.metric}</div>
                        <div className="mt-1 font-label text-[10px] uppercase tracking-[0.18em] text-text-secondary">
                          {project.label}
                        </div>
                      </div>
                      <span aria-hidden="true" className="text-2xl text-primary transition-transform group-hover/card:translate-x-1">→</span>
                    </div>
                  </article>
                </Link>
              ))}
