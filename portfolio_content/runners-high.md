...
implementation: "모드별 Zustand store와 hook을 구성하고 STOMP heartbeat/reconnect, Axios refresh queue, 최근 10초 거리 누적 pace 계산을 적용했습니다."
decision: "실시간 위치는 STOMP publish/subscribe로 분리하고 GPS raw speed 대신 거리 기반 window를 사용했으며, 동시 401 요청은 하나의 refresh queue로 묶었습니다."
evidence: "최근 10초 거리 누적값으로 pace를 계산하고 STOMP heartbeat/reconnectDelay와 Axios refresh queue로 복구 경로를 구성했습니다."
stack: ["TypeScript", "React Native", "Expo", "Zustand", "TanStack Query", "STOMP", "WebSocket", "expo-location", "Axios"]
---
...
- STOMP 클라이언트에는 `heartbeatIncoming`, `heartbeatOutgoing`, `reconnectDelay`를 두고 publish/subscribe 경로를 러닝 모드별로 나눴습니다.
