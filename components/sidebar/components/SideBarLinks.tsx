...
const SideBarLinks: React.FC<SideBarLinksProps> = (props:SideBarLinksProps) => {
  ...
  <div className='w-full'>
      ...
      <Link key = {i} href={`/${PageLinksObj[a.engName]}`} onClick={()=>{props.setSideBar(false)}}><div  className='w-full h-auto py-4 cursor-pointer rounded-lg hover:bg-slate-200'><div className='mx-12 text-md text-black'>{a.korName}({a.engName})</div></div></Link>
        ))
      }
      </div>);
}
